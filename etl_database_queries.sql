...
-- Question 1: does average household income affect the quality of schools? Answer is yes from the query below
...
/* QUESTION 2: Bottom versus Top household income grade score average */
-- Bottom household income grade scores
SELECT ROUND(AVG(cd.household_income)) as "household_income", ROUND(AVG(sg.grade_score_2017),1) as "grade_score_avg"
FROM "census_data" cd, "school_grades" sg 
WHERE cd.household_income > 0
AND cd.zip_code = sg.zip_code
GROUP BY cd.zip_code, cd.household_income
...
/* QUESTION 3: Bottom versus Top household income prices */
...
/* QUESTION 4: Bottom versus top average grade scores by city */
-- Top average grade scores by city
SELECT zhvi.city, ROUND(AVG(sg.grade_score_2017)) as "avg_grade_score"   
FROM "zillow_house_value_index" zhvi, "school_grades" sg 
WHERE zhvi.zip_code = sg.zip_code 
GROUP BY zhvi.city, sg.grade_score_2017
ORDER BY "avg_grade_score" DESC;
-- Bottom average grade scores by city
SELECT zhvi.city, ROUND(AVG(sg.grade_score_2017)) as "avg_grade_score"   
FROM "zillow_house_value_index" zhvi, "school_grades" sg 
WHERE zhvi.zip_code = sg.zip_code 
GROUP BY zhvi.city, sg.grade_score_2017
